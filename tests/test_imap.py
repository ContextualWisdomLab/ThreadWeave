...
def test_serializes_dummy_root_as_nested_sibling_lists():
    """A missing or excluded parent groups sibling branches without an ID."""
    dummy = Container()
...
def test_uid_mode_uses_unique_identifiers():
    """UID THREAD output selects explicit UID metadata instead of sequence data."""
...
def test_root_iterable_is_consumed_once():
    """One-shot root iterables serialize without a hidden second traversal."""
    roots = (_container(identifier) for identifier in (1, 2))
    assert serialize_thread_data(roots) == "THREAD (1)(2)"
...
def test_invalid_emitted_identifiers_are_rejected(identifier: object):
...
def test_non_container_root_is_rejected():
    """Invalid graph roots fail at the public boundary with a useful message."""
...
def test_private_dummy_cannot_appear_as_one_of_multiple_nested_siblings():
    """A malformed private dummy is rejected even beside a concrete sibling."""
    from threadweave.imap import _ResponseNode, _render_thread_list
    ...
    invalid = _ResponseNode(
        ...
        [
            _ResponseNode(2),
            _ResponseNode(None, [_ResponseNode(3), _ResponseNode(4)]),
        ],
    )
    with pytest.raises(ThreadSerializationError, match="only valid at the top level"):
        _render_thread_list(invalid)
