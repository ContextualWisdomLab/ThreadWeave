...
renders the RFC 5256 parenthesized response grammar without recursion or source-
tree mutation. RFC 9051's non-zero unsigned 32-bit message-number bounds are
enforced for both sequence-number and UID output.
"""
...
_RawIdentifierResolver: TypeAlias = Callable[[Message], object]
...
def _project_roots(
    ...
    resolver: _RawIdentifierResolver,
    ...
    or more promoted branches retain their common missing or excluded ancestor as
    a synthetic dummy root, matching RFC 5256's ``((3)(5))`` representation.
    """
    state: dict[int, int] = {}
...
def _render_thread_list(root: _ResponseNode) -> str:
    """Render one RFC ``thread-list`` iteratively with canonical spacing."""
    output: list[str] = []
    stack: list[tuple[str, _ResponseNode | None, bool]] = [("node", root, True)]

    while stack:
        event, node, allow_dummy = stack.pop()
        if event == "close":
            output.append(")")
            continue
...
def serialize_thread_data(
    ...
    *,
    ...
    Raises:
        ThreadSerializationError: The graph is cyclic or shared, or an emitted
            identifier is missing, duplicated, or outside the IMAP range.
        TypeError: A root, child, message, or predicate violates the public model.
        ValueError: ``identifier`` does not name a supported field or callable.
    """
...
def serialize_thread_response(
    ...
    *,
    ...
    ``line_ending`` is restricted to protocol CRLF or an empty string for a
    ...
    data = serialize_thread_data(roots, identifier=identifier, include=include)
    return f"* {data}{line_ending}"
