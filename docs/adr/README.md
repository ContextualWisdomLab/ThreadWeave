...
| [ADR-0008](0008-trusted-release-identity-boundary.md) | Fail closed on an explicitly approved release publisher before irreversible side effects; isolate the current organization PyPI API token to the pinned publisher job and keep Trusted Publishing as an optional credential-minimization mode. | Accepted |
...
| [ADR-0010](0010-actions-registry-lifecycle-evidence.md) | Separate Actions registry observation (read-only audit) from workflow-disable authority (a separate operator action). | Accepted |
