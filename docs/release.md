...
- one dated `CHANGELOG.md` release section;
- one wheel and one source distribution whose filenames contain that version.
...
artifacts, PyPI records, and support documentation unambiguous.

## Job separation

The top-level workflow is intentionally non-reusable because PyPI Trusted
Publishing does not support invoking the publishing action from a reusable
workflow. It uses five distinct jobs:
...
1. **Build** — read-only checkout; install `requirements/ci.lock` with
   ...
   SPDX 2.3 JSON SBOM.
2. **Attest** — download the exact build artifact; generate signed SLSA build
   provenance and an SBOM attestation with GitHub's OIDC-backed `actions/attest`.
3. **Tag** — create an annotated `v<version>` tag only after build and attestation.
   A retry accepts an existing tag only when it points to the same reviewed
   commit.
4. **GitHub Release** — publish the exact distributions, checksum manifest, SPDX
   ...
   release is an idempotent success.
5. **PyPI publish** — download the same artifact into an isolated `pypi`
   environment and invoke the official PyPA publisher with Trusted Publishing.
   PEP 740 publish attestations are enabled; duplicate versions fail instead of
   being silently skipped.

No build or test command runs in the jobs that hold tag, release, attestation, or
PyPI publishing authority.

## SPDX and provenance evidence

`scripts/ci/release_contract.py` emits a deterministic SPDX 2.3 document. It
identifies the ThreadWeave package, declares Apache-2.0, records a PyPI package
URL, and binds both release files to SHA-256 digests. The creation time is the
release date at `00:00:00Z`, not the runner clock, so the metadata can be
reproduced from the reviewed source and artifacts.
...
5. Review each job's exact commit, artifact digest, tag, GitHub Release, PyPI
   provenance, and GitHub attestations.
...
- If a matching tag already exists, the workflow accepts it only when it resolves
  to the current reviewed commit; any other target is a hard failure.
- A GitHub Release retry reuses only a release attached to the verified tag.
...
- Never bypass a failed hash, provenance, environment, or version check with a
  manually uploaded package or long-lived API token.
