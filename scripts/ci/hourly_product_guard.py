"""Fail-closed patch boundary for ThreadWeave's autonomous product workflow.
...
ALLOWED_ROOTS = ("src/threadweave/", "tests/", "docs/")
...
def _forbidden_tokens() -> tuple[bytes, ...]:
    ...
    secret = os.environ.get("THREADWEAVE_FORBIDDEN_SECRET", "").encode("utf-8")
    if not secret:
        return ()
    return tuple(
        token
...
def _reject_forbidden_tokens(data: bytes, *, label: str) -> None:
    """Reject a proposal that contains the model credential or a common encoding."""
...
def _read_proposal(workspace: Path) -> tuple[str, str]:
    ...
    default_title = "ThreadWeave autonomous product increment"
    default_body = (
